...
const DocumentLister = (props) => {
  ...
  const [passMatchedFilesList, setPassMatchedFilesList] = useState([]);
  ...
  const getProjectFiles = async () => {
    ...
    setPassMatchedFilesList(matchedFiles);
    setLoading(false);
  };
  ...
  const filterFiles = (e) => {
    ...
    if (searchText && searchText !== "") {
      const filteredFiles = projectFilesList.filter(
        (x) => x.name.search(searchText) > -1
      );
      setProjectFilesList(filteredFiles);
    }else {
      setProjectFilesList(passMatchedFilesList);
    }
  };
